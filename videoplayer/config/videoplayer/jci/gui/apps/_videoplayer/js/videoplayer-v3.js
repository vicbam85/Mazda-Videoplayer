 ...
 var wsVideo = null;
 var videoTitle = function(title) {
   if (!title) title = $('#myVideoName').text();
   return title.replace(/\.(mp3|mp4|avi|wmv|flv|3gp|flac)/, '');
 }
 ...
 function StartVideoPlayerApp() {
   ...
   $('#toggleBgBtn').click(function() {
     ...
     AIO_SBN("Background: " + ($('#myVideoContainer').hasClass('noBg') ? 'Hide' : 'Show'), videoPlayerIcon);
   ...
   $('#optionStatusbarTitle').click(function() {
     ...
     AIO_SBN("Media Title To Statusbar: " + (statusbarTitleVideo ? 'ON' : 'OFF'), videoPlayerIcon);
     if (!statusbarTitleVideo) { framework.common.setSbName("Video Player") }
   ...
   if (BlackOut) {
     ...
     $('.blackoutTime-left').click(function() {
       $('#blackOutVideoStatus').css('right', '');
     });
     $('.blackoutTime-right').click(function() {
       $('#blackOutVideoStatus').css('right', '0');
     });
   }
 ...
 function myVideoStartRequest(obj) {
   ...
   $('#myVideoStatus, #blackOutVideoStatus').html('');
   $('#blackBgn').css({ 'visibility': '' });
   ...
   } else if (statusbarTitleVideo) {
     framework.common.setSbName(videoTitle());
   }
   AIO_SBN(videoTitle(), videoPlayerIcon);
   ...
   try {
     ...
     wsVideo.onopen = function() {
       ...
       if (CurrentVideoPlayTime) {
         setTimeout(function() {
           wsVideo.send('e 1 t' + (CurrentVideoPlayTime - 1));
           wsVideo.send('h');
         }, 300);
       }
 ...
 function myVideoStopRequest() {
   ...
   $('#videoPlayBtn').css({ 'background-image': '' });
   //$('.VPControlOverlay').css('cssText', 'display: none !important');
 ...
 function toggleBlackOut() {
   ...
   if (BlackOut) {
     $('.VPControlOverlay').addClass('blackOut');
     if (statusbarTitleVideo) {
       $('.VPCtrlAppName, #blackOutVideoStatus').fadeIn();
     }
   } else {
     $('.VPControlOverlay').removeClass('blackOut');
     $('.VPCtrlAppName, #blackOutVideoStatus').fadeOut();
   }
 }
   ...
   if (currentVideoTrack !== null) {
     switch (eventID) {
       ...
       case holdFn2: // default: 'up'
         ...
         if (BlackOut) {
           $('.VPCtrlAppName, #blackOutVideoStatus').fadeToggle();
         }
         break;
